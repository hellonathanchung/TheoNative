...
export function HistoryScreen({ app }: Props) {
  ...
  const avgDuration =
    contractions.length > 0
      ? contractions.reduce((sum, c) => sum + (c.duration ?? 0), 0) / contractions.length
      : 0;
  ...
  return (
    ...
    >
          ...
          {reversedContractions.map((c, reversedIndex) => {
            ...
            return (
              <Pressable
                ...
                style={[
                  s.row,
                  reversedIndex % 2 === 0 && { backgroundColor: Colors.beige },
                ]}
                onPress={() => setSelectedContraction(c)}
              >
      ...
      {sessions.length > 0 && (
        <>
          <Text style={[s.sectionLabel, { marginTop: 24 }]}>PAST SESSIONS</Text>
        ...
        <View>
          {session.contractions.map((c, i) => {
            ...
            return (
              <View
                key={c.id}
                style={[s.row, i % 2 === 0 && { backgroundColor: Colors.beige }]}
              >
